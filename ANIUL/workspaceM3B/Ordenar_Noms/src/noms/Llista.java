...
public class Llista {
	...
	//afegir un objecte a la llista de manera ordenada segons el nom(string)
	public void afegir(Node n)
	{
		Node temp = first;
		int i = 1;
		int comp;
		...
		{
			comp = this.first.nom.compareTo(n.nom);
			
			if(comp > 0)//comprovar si es mes petit que la primera posicio
			{
				afegirInici(n);
			}
			else
			{
				comp = last.nom.compareTo(n.nom);
				
				if(comp < 0)//comprovar si les mes gran que la ultima posicio
				{
					afegirFinal(n);
				}
				else
				{
					while(temp != null && trobat == false)//recoorre la llista buscant la posico que li toca
					{
						comp = temp.nom.compareTo(n.nom);
	...
	public void borrarPosicio(int p)
	{
		...
		if(node_eliminiar == first)
		{
			first = first.next;
		}
		else 
		{
			if(node_eliminiar.next == null)
			{
				this.last = abans;
			}
			else
